      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-gray-700">配信数の推移</h3>
          ...
          <ResponsiveContainer width="100%" height={140}>
            <BarChart data={barData} margin={{ top: 0, right: 0, left: -25, bottom: 0 }}>
              ...
              <Bar dataKey="配信" fill="#3b82f6" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-gray-700">反応率の推移</h3>
            <button className="text-xs border border-gray-200 rounded px-2 py-1 text-gray-500 hover:bg-gray-50 flex items-center gap-1">
              過去30日間
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
            </button>
          ...
          <ResponsiveContainer width="100%" height={140}>
            <LineChart data={lineData} margin={{ top: 0, right: 0, left: -25, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 10, fill: '#9ca3af' }} />
              <Tooltip contentStyle={{ fontSize: 11, borderRadius: 6 }} />
              <Line type="monotone" dataKey="開封率" stroke="#3b82f6" strokeWidth={2} dot={false} name="開封率 (%)" />
              <Line type="monotone" dataKey="返信率" stroke="#10b981" strokeWidth={2} dot={false} name="返信率 (%)" />
            </LineChart>
          </ResponsiveContainer>
          <div className="flex items-center gap-4 mt-1 justify-center">
            {[{ label: '開封率 (%)', color: '#3b82f6' }, { label: '返信率 (%)', color: '#10b981' }].map(l => (
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-gray-700">ステータス別の配信数</h3>
            <button className="text-xs border border-gray-200 rounded px-2 py-1 text-gray-500 hover:bg-gray-50 flex items-center gap-1">
              過去30日間
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
            </button>
          ...
          <div className="flex items-center gap-3">
            <ResponsiveContainer width={110} height={110}>
              <PieChart>
                <Pie data={statusPieData} cx="50%" cy="50%" innerRadius={30} outerRadius={50} dataKey="value" strokeWidth={0}>
            ...
            <div className="flex-1 space-y-1.5">
              {statusPieData.map((item) => (
                <div key={item.name} className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-medium text-gray-700">{item.value.toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="text-center mt-1">
            ...
            <span className="text-sm font-bold text-gray-700">12,450通</span>
          </div>
        </div>
      </div>
